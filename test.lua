...
ScrollingFrame.Position = UDim2.new(-0.00588230789, 0, 0, 0)
...
Eternal_Bob_Hitter.Text = "Auto Hit Eternal Bob (ON/OFF)"
...
local function ZBQFVT_fake_script() -- Eternal_Bob_Hitter.Script 
	...
	local function autoHit()
		while autoHitEnabled do
			local bobBoss = game:GetService("Workspace").bobBoss
			if bobBoss then
				local damageEvent = bobBoss.DamageEvent
				if damageEvent then
	...
	Eternal_Bob_Hitter.MouseButton1Down:Connect(function()
		...
		if autoHitEnabled then
			print("Auto-hit script is turned ON")
			autoHit()
		else
			print("Auto-hit script is turned OFF")
		end
	end)
	
end
coroutine.wrap(ZBQFVT_fake_script)()
local function XZWDF_fake_script() -- Auto_Tycoon.Script 
...
coroutine.wrap(XZWDF_fake_script)()
local function YRFBU_fake_script() -- TextButton3.Script 
...
coroutine.wrap(YRFBU_fake_script)()
local function KIFTZ_fake_script() -- Frame.DragScript 
...
coroutine.wrap(KIFTZ_fake_script)()
